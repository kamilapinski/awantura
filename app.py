from flask import Flask, render_template
from flask_socketio import SocketIO, emit
import random

app = Flask(__name__)
app.config['SECRET_KEY'] = 'sekretny_klucz_awantury'
socketio = SocketIO(app)

QUESTIONS_DB = {
    "Archidiecezja Białostocka": ["Kto jest obecnym metropolitą białostockim?", "W którym roku powstała archidiecezja białostocka?", "Jakiego wezwania jest białostocka katedra?"],
    "Paramenty liturgiczne": ["Do czego służy trybularz?", "Jak nazywa się naczynie do przechowywania konsekrowanych komunikantów w tabernakulum?", "Co to jest puryfikaterz?"],
    "Szaty liturgiczne": ["Jakiego koloru ornatu używa się w okresie Adwentu?", "Czym różni się kapa od ornatu?", "Co symbolizuje humerał?"],
    "Święta i uroczystości": ["W jaki dzień tygodnia zawsze przypada Uroczystość Bożego Ciała?", "Jakie święto kończy okres Bożego Narodzenia w tradycji polskiej?", "Co wspominamy w Wielki Czwartek?"],
    "Budowa kościoła": ["Jak nazywa się miejsce przewodniczenia liturgii przez kapłana?", "Gdzie w kościele przechowuje się Najświętszy Sakrament?", "Czym jest prezbiterium?"],
    "Sakramenty": ["Wymień trzy sakramenty wtajemniczenia chrześcijańskiego.", "Kto jest zwyczajnym szafarzem sakramentu bierzmowania?", "Które sakramenty można przyjąć tylko raz w życiu?"],
    "Księgi liturgiczne": ["Z jakiej księgi diakon lub kapłan czyta Ewangelię podczas Mszy?", "Jak nazywa się księga zawierająca modlitwy mszalne dla celebransa?", "Czym jest lekcjonarz?"],
    "Postawy i gesty": ["Co oznacza postawa stojąca w liturgii?", "W których momentach Mszy Świętej wierni powinni klęczeć?", "Co symbolizuje uderzenie się w piersi?"],
    "Hierarchia kościelna": ["Kto jest głową całego Kościoła katolickiego?", "Wymień trzy stopnie sakramentu święceń.", "Czym zajmuje się Konferencja Episkopatu?"],
    "Czarna Skrzynka": ["Co kryje się w czarnej skrzynce? (Pytanie specjalne)", "Kto z drużyny zje plaster cytryny bez krzywienia się?"]
}

game_state = {
    'teams': {}, 
    'bids': {}, 
    'pool': 0,
    'current_category': "Oczekiwanie na start...",
    'current_question': "",
    'is_spinning': False
}

@app.route('/')
def widownia():
    return render_template('widownia.html')

@app.route('/admin')
def admin():
    return render_template('admin.html', categories=QUESTIONS_DB)

@socketio.on('connect')
def handle_connect():
    emit('update_state', game_state)

@socketio.on('admin_action')
def handle_admin_action(data):
    global game_state
    action = data.get('action')
    team = data.get('team')
    
    try:
        amount = int(data.get('amount', 0))
    except ValueError:
        amount = 0

    if action == 'setup':
        selected_teams = data.get('teams', [])
        game_state['teams'] = {t: amount for t in selected_teams}
        game_state['bids'] = {t: 0 for t in selected_teams}
        game_state['pool'] = 0
        game_state['current_category'] = "Nowa gra rozpoczęta!"
        game_state['current_question'] = ""
        game_state['is_spinning'] = False

    elif action == 'spin_wheel':
        chosen_category = random.choice(list(QUESTIONS_DB.keys()))
        game_state['current_category'] = chosen_category
        game_state['current_question'] = ""
        game_state['is_spinning'] = True
        socketio.emit('spin_animation', {'target_category': chosen_category})

    elif action == 'start_round':
        game_state['is_spinning'] = False # Wyłączamy koło, wchodzi pula i licytacja
        fee = amount
        for t in game_state['teams']:
            if game_state['teams'][t] >= fee:
                game_state['teams'][t] -= fee
                game_state['pool'] += fee
            else:
                game_state['pool'] += game_state['teams'][t]
                game_state['teams'][t] = 0
            game_state['bids'][t] = 0
        game_state['current_question'] = "" # Pytania brak dopóki trwa licytacja

    elif action == 'bid' and team in game_state['teams']:
        current_bid = game_state['bids'].get(team, 0)
        delta = amount - current_bid 
        if delta > 0 and game_state['teams'][team] >= delta:
            game_state['teams'][team] -= delta
            game_state['bids'][team] = amount
            game_state['pool'] += delta

    elif action == 'all_in' and team in game_state['teams']:
        available_funds = game_state['teams'][team]
        current_bid = game_state['bids'].get(team, 0)
        if available_funds > 0:
            game_state['teams'][team] = 0
            game_state['bids'][team] = current_bid + available_funds
            game_state['pool'] += available_funds
            
    elif action == 'add' and team in game_state['teams']:
        game_state['teams'][team] += amount
        
    elif action == 'win_pool' and team in game_state['teams']:
        game_state['teams'][team] += game_state['pool']
        game_state['pool'] = 0
        for t in game_state['bids']:
            game_state['bids'][t] = 0
        game_state['current_question'] = f"Wygrywają {team.upper()}!"

    elif action == 'wrong_answer':
        for t in game_state['bids']:
            game_state['bids'][t] = 0
        game_state['current_question'] = "ZŁA ODPOWIEDŹ! Pula przechodzi do kolejnego pytania!"
            
    elif action == 'show_question':
        game_state['is_spinning'] = False
        game_state['current_question'] = data.get('question', '')

    socketio.emit('update_state', game_state)

if __name__ == '__main__':
    socketio.run(app, debug=True, port=5000)